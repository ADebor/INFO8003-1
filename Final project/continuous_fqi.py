...
class FQI_continuous():
	...
	def __init__(self, n_actions, dataset_episodes,model = None):
		...
		self.dataset = pd.read_pickle("dataset_1000_continuous.pkl")
	...
	def generate_dataset(self,n_episodes):
		#performs n_episodes, and return a dataframe with the one step transitions drawn uniformly
		#between the inteval [-1,1]
		...
		df.to_pickle("dataset_1000_continuous.pkl")
	...
	def learning(self):
		...
		plt.savefig("FQI_continuous_reward_{}iterations_{}_trj.pdf".format(N,self.num_episodes))
		plt.show()
...
if __name__ == "__main__":
	...
	agent = FQI_continuous(n_actions = 41, dataset_episodes = 1000)

	agent.learning()

	pickle.dump(agent.model, open('continuous_fqi.sav', 'wb'))